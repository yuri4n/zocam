---
title: "ADR-001: The refinement-chain model for calendar points"
...
Zocam must represent partial calendar values: "May", "15:00", "the 23rd", "May 2026", "a Wednesday". These values are all "things about time", but they differ in two ways. Some repeat ("May" returns each year) and some do not ("May 2026"). Some name a month, some a day, some a clock time.

One data type must serve all of them. The type must support two operations. First, composition: "May" plus "the 23rd" gives "May 23rd". Second, grounding: a value must map to real intervals on the timeline. The type must also reject nonsense, such as "May of June", as early as possible.
...
The chosen model is the refinement chain. `Zocam.Point` implements it. The precise rules:
...
_Figure 1 — The unit graph: the edges a chain can follow from a scope down to a grain. AI generated, human reviewed._
...
- Fortnight-like rhythms live in the scope as `{:every, k, cycle, anchor}`. See [ADR-004](/design/adrs/004-fortnight-scopes).
...
- `Span.of/1` and the set operators are implemented. The core property — `member?/2` agrees with `ground/3` — stays guarded by the tests in `test/zocam/span_test.exs`. This is not an open item; it is a rule that each later change must keep.
