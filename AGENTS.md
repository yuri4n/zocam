...
- <!-- agent: claude — added 2026-08-05; replaces the earlier sentence that named the workflow as the deployer. --> **Two machines build the docs site, and only one publishes.**
  ...
  - <!-- agent: claude — added 2026-08-09 after the Ask AI button shipped broken. --> One drift already happened, and the fix is in `vercel.json`: Vercel injects `VERCEL_OIDC_TOKEN` at build time, and Docus reads that variable as "the Ask AI assistant has auth". The assistant is a server feature, and a static build has no server, so the button shipped and every question answered with raw HTML. The build command now unsets `VERCEL_OIDC_TOKEN` and `AI_GATEWAY_API_KEY`, which keeps the two builds equal: assistant off on both. To turn the assistant on for real, the site must leave `vercel_static` and get an AI Gateway key — that is a decision, not a flag.
...
- <!-- agent: claude — added 2026-08-09 at the user's request. This is a paired rule. --> **Consult the user's Elixir book corpus before you improvise.** The user keeps the books on disk and wants the work grounded in them: *Designing Elixir Systems with OTP* (Gray & Tate) for the functional-core/process-boundary shape, *Adopting Elixir* (Marx, Valim, Tate) for production judgment, *Programming Phoenix LiveView* and *Building Table Views with Phoenix LiveView* for LiveView work, *Ash Framework* (Le & Daniel) for the platform's applications, *Engineering Elixir Applications* for deploy and operations. Cite the chapter when a book decides something.
